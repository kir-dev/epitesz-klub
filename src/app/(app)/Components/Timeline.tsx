...
const isMedia = (picture: number | Media | null): picture is Media => {
    return picture !== null && typeof picture !== "number" && (picture as Media).url !== undefined
}

export default function Timeline({ events, news }: TimelineProps) {
    ...
    return (
        <div className="bg-black min-h-screen p-8">
            <PageTitle title={news ? "Aktuális" : `${currentYear - 2008} év` } />
                    ...
                    {events.map((item, index) => (
                        <div
                            ...
                            <div className="absolute left-0 top-0 w-8 h-8 -ml-3.5 bg-black border-2 border-[#3E3F46] flex items-center justify-center transform transition-transform duration-300">
                                <span className="text-gray-400 text-sm font-bold">{news ? "" : new Date(item.date).getFullYear().toString().slice(-2)}</span>
                            </div>
                            <div className="ml-12 bg-[#27272A] p-6 shadow-lg border-l-4 border-[#3E3F46] transition-all duration-300 hover:border-gray-400">
                                <h2 className="text-3xl font-bold mb-4 text-gray-200">{news ? item.title : new Date().getFullYear().toString()}</h2>
